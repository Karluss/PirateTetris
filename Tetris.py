...
chest = pygame.image.load('./images/score.png')
...
def drawBox():
    ...
    pygame.draw.rect(screen, BROWN, (top_left_x, top_left_y, play_width, play_height), 4)
...
def drawGrid():
    ...
    y_column = top_left_y
    for row in range(19):
        pygame.draw.line(screen, BROWN, (x_row, y_row), (x_row + play_width, y_row))
        y_row += block_size
    for column in range(11):
        pygame.draw.line(screen, BROWN, (x_column, y_column), (x_column, y_column + play_height))
        x_column += block_size


def drawFalledBlocks():
    ...
    for row in range(20):
        for collumn in range(12):
            if grid[row][collumn] == 1:
                ...
                pygame.draw.rect(screen, DARK_BROWN, (x_block, y_block, block_size, block_size))
...
def createPieces(x, y):
    ...
    J = [[BEIGE, [x, y], [x, y+block_size], [x, y+block_size*2], [x - block_size, y + block_size*2]],
         [BEIGE, [x, y+block_size], [x-block_size, y+block_size], [x-block_size, y], [x+block_size, y+block_size]],
         [BEIGE, [x, y], [x, y + block_size], [x, y + block_size * 2], [x+block_size, y]],
    ...
    L = [[PINK, [x, y], [x, y+block_size], [x, y+block_size*2], [x+block_size, y+block_size*2]],
         [PINK, [x, y+block_size], [x-block_size, y+block_size], [x+block_size, y+block_size], [x-block_size, y+block_size*2]],
    ...
    O = [[ORANGE, [x, y], [x-block_size, y], [x-block_size, y+block_size], [x, y+block_size]]]
    ...
    T = [[GREEN, [x, y], [x, y+block_size], [x+block_size, y+block_size], [x-block_size, y+block_size]],
         ...
         [GREEN, [x, y+block_size*2], [x, y+block_size], [x+block_size, y+block_size], [x-block_size, y+block_size]],
         [GREEN, [x, y], [x, y+block_size], [x-block_size, y+block_size], [x, y+block_size*2]]]
...
def drawPieces(piece, num_piece):
    ...
    pygame.draw.rect(screen, piece[0], (piece[1][0], piece[1][1], block_size, block_size))
    pygame.draw.rect(screen, piece[0], (piece[2][0], piece[2][1], block_size, block_size))
    pygame.draw.rect(screen, piece[0], (piece[3][0], piece[3][1], block_size, block_size))
    pygame.draw.rect(screen, piece[0], (piece[4][0], piece[4][1], block_size, block_size))
...
def canRotate(pieces, pieceIndex, rotation, can_rotate):
    ...
    if rotation == len(pieces[pieceIndex])-1:
        end, collide_x_left, collide_x_right, collide_y, can_rotate = collision(pieces[pieceIndex][0], collide_y, next_rotation)

    elif rotation < len(pieces[pieceIndex])-1:
        end, collide_x_left, collide_x_right, collide_y, can_rotate = collision(pieces[pieceIndex][rotation+1], collide_y, next_rotation)
...
def Score(x, y):
    ...
    text_rect = score_text.get_rect(center=(x_text, y_text))
...
def gameLoop():
    ...
    while not collide_y:
        ...
        pieces, piece, x_atual = checkExcess(piece, x_atual, y_atual, pieceIndex, rotation)
        width_piece, height_piece, min_x, max_x = sizePiece(piece, pieceIndex)
        end, collide_x_left, collide_x_right, collide_y, can_rotate = collision(piece, collide_y, False)
